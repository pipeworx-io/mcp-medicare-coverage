...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/medicare_ncd_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"amyloid PET","limit":25}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/medicare_ncd_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "medicare-coverage": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-medicare-coverage"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-medicare-coverage
```
